...
-- Fixed version that works with existing user_profiles table
...
-- Updated to work with existing user_profiles schema
CREATE OR REPLACE VIEW feedback_topics_with_user_info AS
SELECT 
    ft.*,
    up.full_name AS author_name,
    up.logo_url AS author_avatar,  -- Using logo_url since avatar_url doesn't exist
    CASE 
        WHEN up.subscription_tier = 'agency' THEN 'admin'
        ELSE 'user'
    END AS author_role,  -- Simplified role detection
    EXISTS(
...
LEFT JOIN user_profiles up ON ft.user_id = up.user_id;  -- Note: using user_id not id
...
GRANT SELECT ON feedback_topics_with_user_info TO authenticated;