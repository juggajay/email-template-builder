...
-- Add some public templates if none exist (with proper enum casting)
...
WHERE NOT EXISTS (
  SELECT 1 FROM email_templates WHERE name = 'Welcome Series Template'
);
...
WHERE NOT EXISTS (
  SELECT 1 FROM email_templates WHERE name = 'Holiday Sale Template'
);